...
async function submit(formatMessage, values, dispatch) {
  ...
  if (data.updateSiteSettings.status === "success") {
    toastr.success(formatMessage(messages.updateSiteSettings), formatMessage(messages.commonChangesSuccess));
    dispatch(setSiteSettings());

  } else {
    toastr.error(formatMessage(messages.updateSiteSettings), formatMessage(messages.updateSiteSettingsFail));
  }
