...
class ManageSecurityDeposit extends React.Component {
  ...
  paginationData(currentPage) {
    const {
      getAllReservationAdmin: { refetch },
      setStateVariable,
    } = this.props;
  ...
  changeModalState = (status = false, reservationId) =>
    this.setState({
      claimedModal: status,
      reservationId,
      currentPage: 1,
  ...
  thead = () => {
    ...
    if (claimType == "all")
      claimActionLabel = formatMessage(messages.statusOrAction);
    else if (claimType == "claimed")
      claimActionLabel = formatMessage(messages.status);
    else if (claimType == "nonClaimed")
      claimActionLabel = formatMessage(messages.claimAction);
  ...
  claimStatus = (value) => {
    ...
    let payoutAction = false,
      payoutStatus;
    if (
      value.claimStatus == "approved" &&
      value.claimPayout > 0 &&
      !value.isClaimPaidOut &&
      (value.reservationState === "cancelled" ||
    ...
    if (
      value.reservationState === "expired" ||
      value.reservationState === "declined" ||
      value.claimStatus == "fullyRefunded"
    ) {
      payoutAction = false;
    ...
    return payoutAction ? (
      <a onClick={() => this.openClaimPayoutModal(value)}>{payoutStatus}</a>
    ) : (
      payoutStatus
    );
  };
  ...
  tbody = () => {
    const {
      getAllReservationAdmin: { getAllReservationAdmin },
    } = this.props;
    const { formatMessage } = this.props.intl;
      ...
      return {
        ...
        data: [
            ...
            data: (
              <>
                {value.hostData.firstName +
                  " - " +
                  decode(value.hostData.userData.email)}
              </>
            ),
          },
          {
            data: (
              <>
                {value.guestData.firstName +
                  " - " +
                  decode(value.guestData.userData.email)}
              </>
            ),
          },
          {
            data: (
              <Link
                to={
                  "/siteadmin/viewreservation/" + value.id + "/security-deposit"
                }
              >
          ...
          },
          {
            data: (
              <>
                {value.claimPayout > 0 && value.isClaimPaidOut ? (
                  <CurrencyConverter
          ...
          },
          {
            data: (
              <>
                {value.claimAmount ? (
                  <Link onClick={() => this.changeModalState(true, value.id)}>
            ...
            data: (
              <>
                ...
                (value.claimStatus == "pending" ||
                  value.claimStatus === "requested") ? (
                  <a
                    href="javascript:void(0)"
                    onClick={() =>
                      this.openAdminClaimModal(value.id, value.currency)
                    }
                  >
                  ...
                  value.claimStatus == "fullyRefunded" ? (
                  <span>{formatMessage(messages.claimRefunded)}</span>
                ) : (
                  <div>{formatMessage(messages.messageStatus5)}</div>
                )}
              </>
            ),
          },
  ...
  render() {
    ...
    if (
      ...
      getAllReservationAdmin.reservationData
    ) {
      let reservationData = getAllReservationAdmin.reservationData.find(
      ...
      if (reservationData) {
        modalInitialValues.securityDeposit = convert(
          currencyRates.base,
          currencyRates.rates,
          reservationData.securityDeposit,
          reservationData.currency,
          currencyRates.to
        ).toFixed(2);
        if (
          reservationData.claimStatus &&
          reservationData.claimStatus != "pending"
        ) {
          modalInitialValues.claimAmount = convert(
            currencyRates.base,
            currencyRates.rates,
            reservationData.claimAmount,
            reservationData.currency,
            currencyRates.to
          ).toFixed(2);
    ...
    if (reservationId && adminClaimModal) {
      ...
      if (reservationData) {
        claimedByHost =
          reservationData.claimStatus == "requested" ||
          reservationData.claimStatus == "approved";
        convert(
          currencyRates.base,
          currencyRates.rates,
          reservationData.securityDeposit,
        ...
        adminClaimModalValues.securityDeposit = convert(
          currencyRates.base,
          currencyRates.rates,
          reservationData.securityDeposit,
          reservationData.currency,
        ...
        if (reservationData.claimStatus == "requested")
          adminClaimModalValues.claimAmount = convert(
            currencyRates.base,
            currencyRates.rates,
            reservationData.claimAmount,
            reservationData.currency,
    ...
    return (
      <div className={cx(s.pagecontentWrapper)}>
        <h1 className={s.headerTitle}>
          <FormattedMessage {...messages.manageSecurityDeposit} />
        </h1>
        {claimedModal && (
          <HostClaimModal
            ...
            currency={
              getAllReservationAdmin &&
              getAllReservationAdmin.reservationData.currency
            }
        ...
        <div className={s.marginBottom}>
          ...
          <Button
            onClick={() => this.onFilter("claimed")}
            className={cx(
              cs.btnPrimaryBorder,
              { [s.active]: claimType == "claimed" },
              s.marginLR,
              s.mbFullWidth
            )}
          >
          ...
          <Button
            onClick={() => this.onFilter("nonClaimed")}
            className={cx(
              cs.btnPrimaryBorder,
              { [s.active]: claimType == "nonClaimed" },
              s.mbFullWidth
            )}
          >
