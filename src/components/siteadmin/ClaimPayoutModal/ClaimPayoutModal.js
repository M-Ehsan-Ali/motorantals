...
class ClaimPayoutModal extends Component {
    ...
    submitForm = async (values) => {
        ...
        if (UpdateClaimPayout && UpdateClaimPayout.status == 200) {
            toastr.success(formatMessage(messages.paymentToHost), formatMessage(messages.claimPayoutSuccess));
            refetch();
        } else if (UpdateClaimPayout && UpdateClaimPayout.errorMessage) {
            toastr.error(formatMessage(messages.paymentToHost), UpdateClaimPayout.errorMessage)
        } else {
            toastr.error(formatMessage(messages.paymentToHost), formatMessage(messages.claimPayoutFail));
        }
