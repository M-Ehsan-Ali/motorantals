...
async function submit(formatMessage, values, dispatch) {
  ...
  if (!values.carTripImage1 || !values.carTripImage2) {
    toastr.error(formatMessage(messages.staticBlockSettingsUpdate), formatMessage(messages.updateStaticBlockFail));
    return;
  } 
  ...
  if (data.updateStaticInfoBlock.status === "success") {
    toastr.success(formatMessage(messages.staticBlockSettingsUpdate), formatMessage(messages.commonChangesSuccess));
  } else {
    toastr.error(formatMessage(messages.staticBlockSettingsUpdate), formatMessage(messages.updateStaticBlockFail));
  }
