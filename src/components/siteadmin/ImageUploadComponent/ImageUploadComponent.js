...
export class ImageUploadComponent extends Component {
	...
	error = (file) => {
		...
		if (file && file.size > (1024 * 1024 * parseInt(maxUploadSize))) {
			toastr.error(formatMessage(messages.documentUploadSizeError), formatMessage(messages.documentUploadSizeErrorInfo));
			if (file && (file.accepted === false || fileFormates.indexOf(file.type) >= 0)) {
				toastr.error(formatMessage(messages.commonError), formatMessage(messages.documentUploadFormatError));
				return;
			}
		}

		if(getSiteSettingsLogo && (file && (file.accepted === false || fileFormates.indexOf(file.type) >= 0))){
			toastr.error(formatMessage(messages.commonError), formatMessage(messages.invalidImageUploasPNG));
			return;
		}
	};
...
export default connect(mapState, mapDispatch)(injectIntl(ImageUploadComponent));
