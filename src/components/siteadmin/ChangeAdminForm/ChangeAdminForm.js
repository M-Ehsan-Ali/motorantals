  ...
  async submitForm(values, dispatch) {
    ...
    if (data && data.changeAdminUser) {
      if (data.changeAdminUser.status === '200') {
        toastr.success(formatMessage(messages.changeSuccess), formatMessage(messages.adminAccessChangeSuccess));
      } else if (data.changeAdminUser.status === 'email') {
        toastr.error(formatMessage(messages.commonUpdateFail), formatMessage(messages.adminEmailAlreadyExists));
      } else {
        toastr.error(formatMessage(messages.commonUpdateFail), formatMessage(messages.adminAccessChangeFail));
      }
    }
