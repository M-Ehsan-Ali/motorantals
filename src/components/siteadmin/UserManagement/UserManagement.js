...
class UserManagement extends React.Component {
  ...
  async handleChange(e, userId, userMail, userName) {
    ...
    await updateBanServiceHistoryStatus(
      id,
      banStatus,
      userMail,
      userName,
  ...
  async deleteChange(id, profileId, userType) {
    ...
    await deleteUser(id, profileId, userType, formatMessage);
    this.setState({ currentPage: 1 });
      ...
      return {
        ...
        data: [
          { data: value?.profile?.profileId },
          {
            data: (
              <div
                className={cx(cs.displayFlex, cs.alignCenter, cs.spaceBetween)}
              >
                <a
                  href={
                    "/siteadmin/profileView/" +
                    (value.profile ? value.profile.profileId : "")
                  }
                >
  ...
  render() {
    const {
      userManagement: { loading, userManagement },
    } = this.props;
    const { currentPage, searchList } = this.props;
    const { formatMessage } = this.props.intl;
    return (
