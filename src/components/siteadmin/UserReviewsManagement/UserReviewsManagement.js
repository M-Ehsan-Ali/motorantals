		...
		return data?.reviewsData.map(value => {
			...
			return {
				...
				data: [
					...
					{
						data: !value.isAdminEnable ? <a href="javascript:void(0)" onClick={() => updateReviewStatus(value.id, 'enable', { currentPage, searchList }, formatMessage)}>
							<FormattedMessage {...messages.setToEnable} />
						</a> : <a href="javascript:void(0)" onClick={() => updateReviewStatus(value.id, 'disable', { currentPage, searchList }, formatMessage)}>
