        ...
        data: [
          { data: value?.id, },
          {
            data: <div className={cx(cs.displayFlex, cs.alignCenter, cs.spaceBetween)}>
              {value?.location}
              <TableAction
                onClickDelete={() => deletePopularLocation(value.id, formatMessage)}
                showDelete={true}
                showEdit={true}
          ...
          {
            data: <a href="javascript:void(0)" onClick={() => updateLocationStatus(value.id, value.isEnable, formatMessage)} >
