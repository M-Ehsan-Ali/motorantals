  ...
  async submitForm(values, dispatch) {
    ...
    if (data && data.writeAdminReview) {
      if (data.writeAdminReview.status === '200') {
        if (values.id) {
          toastr.success(formatMessage(messages.commonUpdateSuccess), formatMessage(messages.adminReviewDetailsUpdateSuccess));
        } else {
          toastr.success(formatMessage(messages.commonSubmitSuccess), formatMessage(messages.adminReviewDetailsSubmitSuccess));
          dispatch(reset('AdminReviewsForm'));
        }
      } else if (data.writeAdminReview.status === '404') {
        toastr.error(formatMessage(messages.commonUpdateFail), formatMessage(messages.listIdUnavailable));
      } else {
        toastr.error(formatMessage(messages.commonUpdateFail), formatMessage(messages.toAdminChangeFail));
      }
