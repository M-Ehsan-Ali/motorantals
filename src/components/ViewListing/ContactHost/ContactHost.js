...
class ContactHost extends React.Component {
  ...
  renderWarningBlock(content, error) {
    ...
    return (
      <div
        className={cx(s.alertBlock, bgClass, cs.spaceBottom3, s.mobileMargin)}
      >
        <h4 className={cx(cs.commonContentText)}>{content}</h4>
      </div>
    );
  }
  ...
  render() {
    ...
    return (
      <div>
        <Modal
          show={showContactHostModal}
          onHide={contactHostClose}
          animation={false}
          className={"moreFilterModal"}
        >
            ...
            <div className={cx(s.displayGrid, s.mainPadding)}>
              <div className={cx(s.avatarSection, "contactHostAvatarRTL")}>
                ...
                <div className={s.paddingCommon}>
                  <p className={cx(cs.commonContentText, cs.fontWeightMedium)}>
                    <FormattedMessage {...messages.contactHostinfo1} />:
                  </p>
                  <h6
                    className={cx(
                      cs.commonMediumText,
                  ...
                  >
                    <img
                      src={streeing}
                      className={cx(s.streeingImage, "commonIconSpace")}
                    />
                    <span>
                      <FormattedMessage {...messages.contactHostinfo2} />{" "}
                      {displayName}{" "}
                      <FormattedMessage {...messages.contactHostinfo3} />
                    </span>
                  </h6>
                  <h6
                    className={cx(
              ...
              <div className={cx(s.dateSection, "contactHostDateSectionRTL")}>
                <Loader show={loadingStatus} type={"page"}>
                  <div>
                    {(!isDateChosen ||
                      (!isTimeChosen && !maximumStay && availability)) &&
                      this.renderWarningBlock(
                        <h4 className={cs.commonContentText}>
                          <FormattedMessage {...messages.hostErrorMessage1} />
                        </h4>
                      )}
                    {!maximumStay &&
                      !availability &&
                    ...
                    {isDateChosen &&
                      ...
                      this.renderWarningBlock(
                        <h6 className={cs.commonContentText}>
                          <FormattedMessage {...messages.maximumStay} />{" "}
                          {maxDay} <FormattedMessage {...messages.nights} />
                        </h6>,
                        "error"
                      )}
                    {availability &&
                      ...
                      this.renderWarningBlock(
                        <p className={cs.commonContentText}>
                          <FormattedMessage {...messages.hostErrorMessage3} />
                        </p>
                      )}
                    {isTimeChosen &&
                      !isValue &&
                      this.renderWarningBlock(
                        <div className={s.errorCss}>
                          <FormattedMessage
                            {...messages.chooseDifferentEndTime}
                          />
                        </div>
                      )}
                    <Form
                      onSubmit={handleSubmit(
                        submit.bind(null, this.props.intl)
                      )}
                    >
                      ...
                      <Button
                        className={cx(cs.btnPrimary, cs.btnLarge, cs.fullWidth)}
                        type="submit"
                      ...
                      >
                        ...
                        <img
                          src={arrow}
                          className={cx(s.arrowIconCss, "sliderArrowRTL")}
                        />
                      </Button>
                    </Form>
                  </div>
...
export default injectIntl(
  withStyles(s, cs, c)(connect(mapState, mapDispatch)(ContactHost))
);
