...
class HostClaimModal extends React.Component {
    ...
    async handleSubmit(values, dispatch) {
        ...
        if (data && data.updateClaimForHost && data.updateClaimForHost.status == 200) {
            toastr.success(formatMessage(messages.commonSuccess), formatMessage(messages.claimRequwestSuccess))
            this.closeModal();
        } else {
            toastr.error(formatMessage(messages.errorOops), formatMessage(messages.somethingWrong));
    ...
    async removeImage(image) {
        ...
        if (data && data.status == 200) {
            ...
            toastr.success(formatMessage(messages.commonSuccess), formatMessage(messages.imageRemoveSuccess));
