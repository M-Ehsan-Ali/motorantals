...
class DocumentList extends Component {
  ...
  async handleClick(id, fileName) {
    ...
    if (
      data &&
      data.RemoveDocumentList &&
      data.RemoveDocumentList.status == "success"
    ) {
      ...
      toastr.success(
        ...
        formatMessage(messages.documentRemoveSuccess)
      );
    }
  }
  ...
  render() {
    ...
    console.log('ShowDocumentList:', ShowDocumentList)
      ...
      <div className={cx("row")}>
        {ShowDocumentList &&
          ShowDocumentList.map((item, key) => {
            let icon =
              item.fileType == "application/pdf" ? pdfIcon : item.fileName;
            console.log({ icon });
            return (
              <div
                key={key}
