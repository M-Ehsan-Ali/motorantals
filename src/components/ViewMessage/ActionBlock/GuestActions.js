        ...
        if (data && data.checkReservation) {
            if (data.checkReservation.status == "200") {
                if (listPublishStatus) {
                    ...
                    toastr.error(formatMessage(messages.commonSorry), formatMessage(messages.unpublishedOrDeletedListing));
                }
            }
            else {
                toastr.error(formatMessage(messages.bookingFailure), formatMessage(messages.bookingDatesUnavailable));
            }
        }
    }
