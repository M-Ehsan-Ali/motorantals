...
class Payment extends Component {
  ...
  componentDidMount() {
    toastr.info(
      "IMPORTANT: The actual payment will be made in Thai Baht. The price in your selected currency is for guidance and informational purposes only."
    );
  }
  ...
  render() {
        ...
        if (stayedNights && stayedNights.length > 0) {
          stayedNights.map((item, key) => {
            ...
            if (item) {
              ...
              isSpecialPricing =
                bookingData &&
                bookingData.listBlockedPrice.find(
                  (o) =>
    ...
    total =
      priceForDays + guestServiceFee + delivery + securityDeposit - discount;
    //add total to graphql

    totalWithoutFees = priceForDays + delivery - discount;

    let checkInDate =
      checkIn != null ? moment(checkIn).format("ddd, MMM D, YYYY") : "";
    let checkOutDate =
      checkOut != null ? moment(checkOut).format("ddd, MMM D, YYYY") : "";
      ...
      <Grid fluid className={s.container}>
        <Row className={cs.positionRelative}>
          <Col
            ...
            sm={12}
            xs={12}
            className={cx(cs.spaceTop7, "paymentDetailsPadding")}
          >
              <PaymentForm
          ...
          >
            <div className={s.datePanelSection}>
              ...
              {starRatingValue > 0 && (
                <div
                  className={cx(cs.spaceTop1, cs.commonContentText, s.starFlex)}
                >
              ...
              <div className={s.tableFlex}>
                ...
                <img
                  src={arrowIcon}
                  className={cx(s.arrowMargin, "commonDateArrowRTLRotate")}
                />
                <div>
                  <h5
                    className={cx(
