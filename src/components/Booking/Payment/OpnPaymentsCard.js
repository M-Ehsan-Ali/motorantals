...
import krungsribank from "/public/SiteIcons/kbank.png";
import baybank from "/public/SiteIcons/krungsriBank.png";
  ...
  return (
    <>
      <div className={s.paymentMethod} onClick={onClick}>
        ...
        <img
          style={{ width: 24 }}
          src={isNextType ? chevronIcon : rightTopArrowIcon}
        />
      </div>
    </>
  );
...
const PaymentScreen = ({ icon, name }) => {
  console.log(messages.methodPaymentDescription);
    ...
    <>
      <div>
        ...
        <div className={s.paymentScreenText}>
          <FormattedMessage
            {...messages.methodPaymentDescription}
            values={{ name }}
          />
        </div>
      </div>
  ...
  const handleChange = (value, type) => {
    if (type === "cardNumber") {
      ...
      if (isNaN(value)) {
        ...
        dispatch(
          change("PaymentForm", "creditCardNumber", formatCardNumber(value))
        );
      }
                      ...
                      return (
                        <div>
                          <select
                            ...
                            onChange={(e) =>
                              handleChange(e.target.value, "cardMonth")
                            }
                          >
                      ...
                      return (
                        <div>
                          <select
                            ...
                            onChange={(e) =>
                              handleChange(e.target.value, "cardYear")
                            }
                          >
                            {" "}
                            <option value="0" disabled>
            ...
            <div className={s.otherPaymentMethods}>
              {paymentMethods.map((method, idx) =>
                method.value === "googlepay" ? (
                  <div>
                    <GoogleButton
                      style={{ borderBottom: "1px solid rgb(208, 214, 226)" }}
                    >
                      <PaymentMethod
                        label={method.label}
                        value={method.value}
