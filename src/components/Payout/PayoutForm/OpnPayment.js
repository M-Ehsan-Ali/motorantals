...
// Components
import generateOpnPayoutToken from "../../../helpers/generateOpnPayoutToken";
import Loader from "../../Loader/Loader";

class OpnPayment extends Component {
  static propTypes = {
  ...
  async handleSubmitAction(event) {
    ...
    const { Opn, change, handleSubmit } = this.props;
    const {
      formValues,
      errors,
      startPayoutLoading,
    ...
    if (
      ...
      const generateTokens = await generateOpnPayoutToken(Opn, formValues);
    ...
    return (
      ...
      >
        <form onSubmit={handleSubmit}>
          ...
          {payoutCountry &&
            ["US", "CA", "GB"].indexOf(payoutCountry) !== -1 && (
              <Field
                name="routingNumber"
                component={this.renderField}
                label={
                  payoutCountry === "GB"
                    ? formatMessage(messages.payoutSortNumber)
                    : formatMessage(messages.payoutRouting)
                }
                placeHolder={
                  payoutCountry === "CA"
                    ? "eg: 11000-000"
          ...
          <Field
            ...
            label={
              isEuropeCountry(payoutCountry)
                ? formatMessage(messages.ibanNumber)
                : formatMessage(messages.accountNumber)
            }
            placeHolder={
              isEuropeCountry(payoutCountry)
                ? `eg: ${payoutCountry}89370400440532013000`
                : payoutCountry === "GB"
                ? "eg: 00012345"
                : "eg: 000123456789"
            }
          ...
          <Field
            ...
            label={
              isEuropeCountry(payoutCountry)
                ? formatMessage(messages.confirmIbanNumber)
                : formatMessage(messages.confirmAccountNumber)
            }
            placeHolder={
              isEuropeCountry(payoutCountry)
                ? `eg: ${payoutCountry}89370400440532013000`
          ...
          <p className={cx(cs.commonMediumText, cs.paddingBottom2)}>
            {formatMessage(messages.payoutOpnInfo)}
          </p>
          <p className={cs.commonMediumText}>
            {formatMessage(messages.opnTokenInfo1)}{" "}
            <a
              href={"https://www.opn.ooo/th-en/legal"}
              target={"_blank"}
              className={cs.siteLinkColor}
            >
              {formatMessage(messages.opnTokenInfo2)}
            </a>
            .
          </p>
...
export default injectIntl(
  withStyles(s, cs)(connect(mapState, mapDispatch)(OpnPayment))
);
