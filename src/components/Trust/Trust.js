...
class MenuComponent extends React.Component {
  ...
  sendConfirmEmail() {
    ...
    resendEmailVerification(this.props.intl);
  }
  ...
  render() {
    ...
    let displayVerifiedPanel =
      isEmailConfirmed || isGoogleConnected || isIdVerification || false;
    let displayUnVerifiedPanel =
      !isEmailConfirmed || !isGoogleConnected || !isIdVerification || false;
    let allUnverified =
      ...
      <div className={cx(cs.commonBorderSection, "whiteBgColor")}>
        {displayVerifiedPanel && (
          <>
            ...
            >
              {formatMessage(messages.verifiedInfo)}
            </h4>
          ...
          <>
            <h4
              className={cx(
                cs.commonTotalText,
                cs.fontWeightBold,
                { [cs.paddingBottom2]: allUnverified },
                { [cs.paddingBottom4]: displayVerifiedPanel }
              )}
            >
              {formatMessage(messages.notVerifiedInfo)}
            </h4>
            {/* {
            ...
            {!isEmailConfirmed && !isGoogleConnected && !isIdVerification && (
              <p className={cx(cs.paddingBottom4, s.marginBottmNone)}>
                <FormattedMessage {...messages.notVerifiedDetails} />
              </p>
            )}
            <div className={cx(s.cardSecBg)}>
              {!isEmailConfirmed && (
...
export default injectIntl(
  withStyles(s)(connect(mapState, mapDispatch)(MenuComponent))
);
