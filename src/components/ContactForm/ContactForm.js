...
class ContactForm extends Component {
  ...
  async handleClick(values, dispatch) {
    ...
    if (status === 200) {
      toastr.success(
        ...
        formatMessage(messages.emailHasBeenSent)
      );
    } else {
      toastr.error(
  ...
  renderCaptcha = ({
    ...
    return (
      <div>
        <ReCAPTCHA sitekey={siteKey} onChange={input.onChange} />
        {touched && error && (
          <span className={s.errorMessage}>{formatMessage(error)}</span>
        )}
      </div>
    ...
    return (
      <Grid fluid>
        <Row>
                  ...
                  <Col lg={4} md={4} sm={4} xs={12} className={s.alignCenter}>
                    <div className={s.space6}>
                      <div>
                        ...
                        <div>
                          <h1 className={cx(s.contactTitle, s.subTitleText)}>
                            <FormattedMessage {...messages.contactFormCall} />
                          </h1>
                          <a
                ...
                {address && (
                  <Col lg={4} md={4} sm={4} xs={12} className={s.alignCenter}>
                    <div className={s.space6}>
                      <div>
                        <div className={s.iconMargin}>
                          <img src={qrcode} className={s.mailIcon} />
                        </div>
                        <h1 className={cx(s.contactTitle, s.subTitleText)}>
                          {/*<FormattedMessage {...messages.contactFormAddress} />*/}
                          <FormattedMessage {...messages.contactFormReactUsAsTitle} />
                        </h1>
          ...
          <h4 className={s.addressText} style={{marginTop: '100px', textAlign: 'center'}}>{address}</h4>
                ...
                <form onSubmit={handleSubmit(this.handleClick)}>
                    ...
                    <Col xs={12} sm={6} md={6} lg={6} className={s.noPadding}>
                      <Col xs={12} sm={12} md={12} lg={12}>
                        <label className={s.labelText}>
                          {formatMessage(messages.Nameincontact)}
                        </label>
                      </Col>
                      <Col xs={12} sm={12} md={12} lg={12}>
                        <Field
                  ...
                  <Row className={s.formGroup}>
                    <Col
                      ...
                      md={12}
                      lg={12}
                      className={cx(s.overFlowHidden, s.spaceTop1)}
                    >
                      <Field name="reCaptcha" component={this.renderCaptcha} />
                    </Col>
                  </Row>
                  <Row className={s.formGroup}>
                    <Col
...
export default injectIntl(
  withStyles(s)(connect(mapState, mapDispatch)(ContactForm))
);
