...
class SaveButton extends Component {
  ...
  async handleSubmit() {
    ...
    if (step === 1 && isEmpty(step1Errors)) {
      if (formPage === "location") {
        let isHeader = true;
        await updateListingMap(isHeader, formatMessage);
      } else {
      ...
      toastr.error(
        ...
        formatMessage(messages.saveBtnError)
      );
    }
  }
  ...
  render() {
    ...
    if (step === 1) {
      if (
        listingSteps.step1 != undefined &&
        listingSteps.step1 === "active"
      ) {
        visible = true;
      }
    }
    if (step === 2) {
      if (
        listingSteps.step2 != undefined &&
        listingSteps.step2 === "active"
      ) {
        visible = true;
      }
    }
    if (step === 3) {
      if (
        listingSteps.step3 != undefined &&
        listingSteps.step3 === "active"
      ) {
        visible = true;
      }
    }
    ...
    return (
      <div
        ...
        className={cx("textLoaderSave", cs.curserPointer)}
      >
        <span className={className}>
          {visible && updateListing && (
            <span>
              <span className={s.nonVerySmallScreen}>
                <Loader
                  loadingText={formatMessage(messages.savingButton)}
                  type={"text"}
                />
              </span>
            </span>
          ...
          {!updateListing && visible && (
            <span>
              <span
                className={cx(
                  s.nonVerySmallScreen,
                  cs.siteLinkColor,
              ...
              >
                {!isSaving && !isBlocking && (
                  <>
                    ...
                    <img src={arrow} className={"saveArrowCss"} />
                  </>
                )}
              </span>
            </span>
...
export default injectIntl(
  withStyles(s, cs)(connect(mapState, mapDispatch)(SaveButton))
);
