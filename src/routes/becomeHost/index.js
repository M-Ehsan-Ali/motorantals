...
export default async function action({ store, query, params }) {
  ...
  let adminPrivileges =
    store.getState().account.privileges &&
    store.getState().account.privileges.privileges;
  let privileges =
    store.getState().listSettings && store.getState().listSettings.privileges;
  const listingPages = [
    "index",
    "home",
    "car",
  ...
  if (
    ...
    !restrictUrls("/become-a-owner/", adminPrivileges, privileges)
  ) {
  ...
  if (listId != undefined && !isNaN(listId)) {
    let step;
    const step1Pages = ["car", "location", "map", "features"];
    const step2Pages = ["photos", "description"];
    const step3Pages = [
      "car-rules",
      "review-how-renters-book",
      "min-max-days",
