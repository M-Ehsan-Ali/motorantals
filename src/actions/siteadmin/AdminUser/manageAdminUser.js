    ...
    return async (dispatch, getState, { client }) => {
        ...
        try {
            ...
            if (data && data.createAdminUser && data.createAdminUser.status === 200) {
                ...
                toastr.success(formatMessage(messages.adminUserLabel), "Admin User has been " + (id ? 'updated' : 'added') + " successfully!");
                return {
                    status: 200
                };
            } else {
                toastr.error(formatMessage(messages.adminUserLabel), "Oops, something went wrong" + data && data.createAdminUser && data.createAdminUser.errorMessage);
                await dispatch({
...
export function deleteAdminUser(id, formatMessage) {
    return async (dispatch, getState, { client }) => {
        await dispatch({
        ...
        try {
            ...
            if (data && data.deleteAdminUser && data.deleteAdminUser.status === 200) {
                ...
                dispatch(closeAdminUserModal());
                toastr.success(formatMessage(messages.adminUserLabel), formatMessage(messages.adminUserDeleteSuccess));

            } else {
                toastr.error(formatMessage(messages.adminUserLabel), "Oops, something went wrong" + data && data.deleteAdminUser && data.deleteAdminUser.errorMessage);
                await dispatch({
