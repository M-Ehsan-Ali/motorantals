    ...
    return async (dispatch, getState, { client }) => {
        ...
        try {
            ...
            if (data && data.createAdminRole && data.createAdminRole.status === 200) {
                ...
                toastr.success(formatMessage(messages.adminRoleLabel), "Admin Role has been " + (id ? 'updated' : 'added') + " successfully!");
                return {
                    status: 200
                };
            } else {
                toastr.error(formatMessage(messages.adminRoleLabel), "Oops, something went wrong" + data && data.createAdminRole && data.createAdminRole.errorMessage);
                dispatch(closeAdminRolesModal());
                await dispatch({
...
export function deleteAdminRole(id, formatMessage) {
    return async (dispatch, getState, { client }) => {
        await dispatch({
        ...
        try {
            ...
            if (data && data.deleteAdminRole && data.deleteAdminRole.status === 200) {
                ...
                dispatch(closeAdminRolesModal());
                toastr.success(formatMessage(messages.adminRoleLabel), formatMessage(messages.adminRoleDeleteSuccess));
                return {
                    status: 200
                };
            } else {
                toastr.error(formatMessage(messages.adminRoleLabel), "Oops, something went wrong" + data && data.deleteAdminRole && data.deleteAdminRole.errorMessage);
                await dispatch({
