...
export function deleteUser(userId, profileId, formatMessage) {
  ...
  return async (dispatch, getState, { client }) => {
    ...
    try {
      ...
      if (data.deleteUser.status == "success") {
        ...
        toastr.success(formatMessage(messages.deleteUser), formatMessage(messages.deleteUserSuccess));
        history.push('/siteadmin/users');
      } else if (data.deleteUser.status == "activebooking") {
        dispatch({
          type: ADMIN_DELETE_USER_ERROR,
        });
        toastr.error(formatMessage(messages.deleteUser), data.deleteUser.errorMessage);
      } else {
        toastr.error(formatMessage(messages.deleteUser), formatMessage(messages.deleteUserFail));
      }
