...
export function updateListingMap(isHeader, formatMessage) {
  return async (dispatch, getState, { client }) => {
    ...
    try {
      ...
      console.log("data.locationItem:", data.locationItem);
      ...
      if (data && data.locationItem) {
        if (data.locationItem.status !== 200) {
          toastr.error(
            formatMessage(messages.invalidAddressInfo),
            formatMessage(messages.editAddressInfo)
          );
          dispatch({
