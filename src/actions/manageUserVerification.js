...
export function resendEmailVerification(intl) {
  ...
  return async (dispatch, getState, { client }) => {
    ...
    try {
      ...
      if (data.ResendConfirmEmail.status !== "error") {
        ...
        if (status === 200) {
          toastr.success(
            formatMessage(messages.emailIsSent),
            formatMessage(messages.confirmationLinkSent)
          );
        } else {
          toastr.error(
            formatMessage(messages.emailFail),
