...
export function importiCal(listId, name, url, calendarId, formatMessage) {
    ...
    return async (dispatch, getState, { client }) => {
        ...
        try {
            ...
            if (status === 200) {
                toastr.success(formatMessage(messages.calendarAddSuccess), formatMessage(messages.calendarAddSuccessInfo));
                if (!calendarId) {
                ...
                if (status === 409) {
                    toastr.error(formatMessage(messages.calendarExists), formatMessage(messages.calendarExistsInfo));
                } else {
                    toastr.error(formatMessage(messages.calendarInvalid), formatMessage(messages.calendarInvalidInfo));
                }
                dispatch({
                    type: IMPORT_CALENDAR_ERROR,
                    payload: {
