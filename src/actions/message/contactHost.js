    ...
    try {
      ...
      if (startTime.includes(".")) {
        timeStart =
          startTime && startTime.length == "3"
            ? "0" + startTime.slice(0, 1) + ":30:00"
            : startTime && startTime.slice(0, 2) + ":30:00";
      } else {
        timeStart =
          startTime && startTime.length == "1"
            ? "0" + startTime.slice(0, 1) + ":00:00"
            : startTime && startTime.slice(0, 2) + ":00:00";
      }

      if (endTime.includes(".")) {
        timeEnd =
          endTime && endTime.length == "3"
            ? "0" + endTime.slice(0, 1) + ":30:00"
            : endTime && endTime.slice(0, 2) + ":30:00";
      } else {
        timeEnd =
          endTime && endTime.length == "1"
            ? "0" + endTime.slice(0, 1) + ":00:00"
            : endTime && endTime.slice(0, 2) + ":00:00";
      }
      ...
      if (data && data.CreateThreadItems) {
        ...
        toastr.success(
          ...
          formatMessage(messages.requestSentSuccess)
        );

        sendEmail(hostEmail, "inquiry", emailContent, intl);
      }
    } catch (error) {
      dispatch({
