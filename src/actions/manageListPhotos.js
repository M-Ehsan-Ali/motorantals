...
export function createListPhotos(listId, name, type, formatMessage) {
  ...
  return async (dispatch, getState, { client }) => {
    ...
    try {
      ...
      if (data && data.CreateListPhotos && data.CreateListPhotos.status === 'success') {
        ...
        toastr.success(formatMessage(messages.commonSuccess), formatMessage(messages.photoUploadSuccess));
      }
...
export function removeListPhotos(listId, name, reload, formatMessage) {
  ...
  return async (dispatch, getState, { client }) => {
    ...
    `;
        ...
        if (showListPhotosData && showListPhotosData.data.ShowListPhotos && showListPhotosData.data.ShowListPhotos.length <= 1) {
          // If length less 
          toastr.error(formatMessage(messages.errorOccured), formatMessage(messages.errorOccured));
