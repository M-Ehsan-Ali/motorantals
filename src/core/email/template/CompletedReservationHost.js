...
class CompletedReservationHost extends React.Component {
  ...
  render() {
    ...
    const {
      content: { guestName, guestLastName, guestProfilePic },
    } = this.props;
              ...
              <TR>
                <TD style={textStyle}>
                  ...
                  <h1 style={bookingTitle}>
                    `${formatMessage(messages.contactHostinfo2)} ${guestName} $
                    {guestLastName} ${formatMessage(messages.whatYouLoved)}`{" "}
                    <br />
                    <span>
                      {" "}
                  ...
                  <EmptySpace height={20} />
                  <div>
                    {`${guestName} ${formatMessage(messages.endedHisTrip)}`}
                  </div>
                  <EmptySpace height={20} />
                  <div>
                    `${formatMessage(messages.reviewsImportantPart)} ${sitename}{" "}
                    ${formatMessage(messages.plsTakeAMomentRenter)}`
                  </div>
...
export default injectIntl(CompletedReservationHost);
