...
class ForgotPasswordEmail extends React.Component {
  ...
  render() {
    ...
    let verificationURL =
      url + `/en-US/password/verification?token=${token}&email=${email}`;
    return (
      <Layout>
        <Header
          color="rgb(255, 90, 95)"
        ...
        <Body textStyle={textStyle}>
          ...
          <div>
            <FormattedMessage {...messages.receivedToResetPassword} />
          </div>
          <EmptySpace height={20} />
          <div>
            <FormattedMessage {...messages.notMakeReq} />
          </div>
          <EmptySpace height={40} />
          <div>
            <a style={buttonStyle} href={verificationURL}>
              <FormattedMessage {...messages.clickResetPass} />
            </a>
          </div>
...
export default injectIntl(ForgotPasswordEmail);
