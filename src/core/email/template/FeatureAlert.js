    ...
    return (
      <Layout>
        ...
        <Body textStyle={textStyle}>
          ...
          <div>
            {`${email} ${formatMessage(messages.subscribedToNoficitation)}`}{" "}
            <span style={textBold}>{feature}</span>{" "}
            {`${formatMessage(messages.at)} ${today}`}
          </div>
...
export default injectIntl(FeatureAlert);
